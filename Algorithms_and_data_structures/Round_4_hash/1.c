...
int main(void) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!find(number)) {
            store_hash(number);
            array[len_array++] = number;
        }
    }

    fwrite(&len_array, sizeof(int), 1, out);
    for (int j = 0; j < len_array; ++j) {
        fwrite(&array[j], sizeof(int), 1, out);
    }
    fclose(in);
    fclose(out);
    return 0;
