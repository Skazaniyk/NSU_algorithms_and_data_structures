...
int value_in_tree(tree *t, ll value) {
    if (t != NULL) {
        if (t->value == value) {
            return 1;
        }
...
void findmin_value(tree *t, ll value) {
    /*if (t == NULL) {
        return INT_MAX;
    }
    if (t->value >= value) {
...
int main() {
    ...
    ll m, number, res;
    char string[10], null_string[10000], add[3] = "add", remove[6] = "remove", lower[5] = "lower";
    scanf("%lld", &m);
    tree *t = NULL;
    tree *p;
    gets(null_string);
    for (int i = 0; i < m; i++) {
        gets(null_string);
        sscanf(null_string, "%s %lld", string, &number);
        if (string[0] == add[0]) {
