...
int main() {
    ...
    Graph *graph = createGraph(n * m + 5);
    for (int i = 0; i < n; i++) {
        scanf("%c", &symbol);
        array[i] = malloc(sizeof(int) * (m + 5));
        for (int j = 0; j < m; j++) {
            scanf("%c", &symbol);
            if (symbol == 'X') {
                array[i][j] = 1;
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (array[i][j] == 1) {
                graph->visited[i * m + j] = 0;
            } else {
                if (i > 0 && array[i - 1][j] == 0) {
                    addEdge(graph, i * m + j, i * m + j - m);
                }
                if (i < n - 1 && array[i + 1][j] == 0) {
                    addEdge(graph, i * m + j, i * m + j + m);
                }
                if (j > 0 && array[i][j - 1] == 0) {
                    addEdge(graph, i * m + j, i * m + j - 1);
                }
                if (j < m - 1 && array[i][j + 1] == 0) {
                    addEdge(graph, i * m + j, i * m + j + 1);
                }
            }
        }
    ...
    if (graph->visited[finish] == MAX) {
        printf("-1");
    } else {
        printf("%d", graph->visited[finish]);
    }
